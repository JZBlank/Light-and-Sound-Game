...
function guess(btn){
  ...
  //add game logic here
  if (btn == pattern[guessCounter]){ //if the button pressed is correct 
    if (guessCounter == progress){ //keeps track of how much sequence user has done correctly
        if (progress == pattern.length-1){ // if user guesses everything correct
          winGame(); // user wins
        }
        else{ //Sequence guessing is not done
          progress++; // increment guess counter
          playClueSequence();
        }
    }
    else{ // keep guessing 
  ...
  //If wrong button is pressed, lose game
     loseGame();
  }
}
